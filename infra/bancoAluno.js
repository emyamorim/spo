...
alunoBanco.prototype.busca2 = function(rm_aluno, callback) {
  this._conexao.query(
    'select * from aluno where rm_aluno like ?',
    rm_aluno,
    callback
  );
};
...
alunoBanco.prototype.editar = function(dados, callback) {
  this._conexao.query(
    'UPDATE aluno SET senha_aluno=? WHERE rm_aluno=?',
    [dados, dados.rm_aluno],
    callback
  );
};
...
alunoBanco.prototype.editardados = function(dados, callback) {
  this._conexao.query(
    'UPDATE aluno SET ? WHERE rm_aluno=?',
    [dados, dados.rm_aluno],
    callback
  );
};
...
alunoBanco.prototype.login = function(dados, callback) {
  this._conexao.query(
    'SELECT * FROM aluno WHERE rm_aluno = ? and senha_aluno=?',
    [dados.rm_aluno, dados.senha_aluno],
    callback
  );
};
